      ...
      <table className="min-w-full text-left text-sm">
        <thead className="bg-slate-900 text-slate-400">
          <tr>
            ...
            <th className="px-4 py-3">ライン</th>
            <th className="px-4 py-3">症状</th>
            {!compact && <th className="px-4 py-3">原因</th>}
            <th className="px-4 py-3">処置</th>
            {!compact && <th className="px-4 py-3">担当</th>}
        ...
        <tbody>
          {records.map((r) => (
            <tr key={r.id} className="border-t border-slate-800 hover:bg-slate-900/50">
              ...
              <td className="px-4 py-3 font-medium text-emerald-400">{r.equipment_name ?? "—"}</td>
